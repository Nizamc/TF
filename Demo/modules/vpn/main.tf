...
resource "azurerm_public_ip" "pip_aa" {
  ...
  name                = lower("${var.vpn_gateway_name}-${var.resource_group_name.rg.location}-gw-aa-pip")
  ...
  tags                = merge({ "ResourceName" = lower("${var.vpn_gateway_name}-${var.resource_group_name.rg.location}-gw-aa-pip") }, var.tags, )
}
  ...
  dynamic "vpn_client_configuration" {
    ...
    content {
      ...
      root_certificate {
        ...
        #public_cert_data = filebase64("./modules/certificates/myvpnroot.cer")
      }
